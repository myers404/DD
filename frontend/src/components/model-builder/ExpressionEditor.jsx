...
// Enhanced lexer with position tracking
class Lexer {
  constructor(input) {
    this.input = input;
    this.position = 0;
...
// Evaluator for runtime evaluation
class Evaluator {
...
const ValidationDisplay = ({ result, executionTime }) => {
    ...
    <div className={`p-3 rounded-lg ${
      ...
      <div className="flex items-center">
        ...
        <span className={`text-sm font-medium ${
          ...
          {result.success ? 'Expression is valid' : 'Expression has errors'}
        </span>
        {result.success && executionTime && (
          <span className="text-xs text-green-600 ml-2">({executionTime}ms)</span>
        )}
      </div>
      ...
      {result.success && typeof result.result !== 'undefined' && (
  ...
  return (
    <div className="bg-yellow-50 rounded-lg p-3 space-y-2">
      <div className="flex items-center space-x-2">
        ...
        <span className="text-sm font-semibold text-gray-900">Code Analysis</span>
      </div>
    ...
    <div className="bg-gray-50 rounded-lg p-3 space-y-3">
      ...
      <div className="space-y-2">
        {variables.map(variable => (
          <div key={variable.name} className="bg-white p-2 rounded border space-y-1">
            <div className="flex items-center justify-between">
              <span className="font-mono text-green-600 text-sm font-medium">{variable.name}</span>
...
const ExpressionEditor = ({ 
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(parseExpression, 500);
    return () => clearTimeout(timeoutId);
  }, [value, variables, validator, onValidationChange]);
  ...
  useEffect(() => {
    const match = findMatchingBracket(value, cursorPosition);
  ...
  const handleVariableChange = useCallback((name, newValue) => {
    // Notify parent if there's a callback
    if (onChange) {
      // This is a simplified approach - in a real implementation,
      // you might want to have a separate callback for variable changes
      console.log(`Variable ${name} changed to:`, newValue);
    }
  }, [onChange]);
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="bg-white rounded-lg border border-gray-300 shadow-sm">
        <div className="border-b border-gray-200 px-4 py-2 flex items-center justify-between">
          <h3 className="font-medium text-gray-900">Expression Editor</h3>
          <button
            type="button"
        ...
        <div className="relative">
          <textarea
            ...
            className="w-full h-32 p-4 font-mono text-sm resize-none border-0 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-transparent relative z-10 leading-6"
        ...
        {showValidation && (
          <div className="border-t border-gray-200 p-4">
            <ValidationDisplay 
              result={evaluationResult || parseResult} 
      ...
      {showAutoComplete && (
        <AutoCompletePopup
          ...
          onSelect={() => {}} // Simplified for this version
